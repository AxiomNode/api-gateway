...
export type PersistedRoutingServiceKey = "ai-engine-api" | "ai-engine-stats";
...
function defaultStateFilePath(): string {
  return path.resolve(process.cwd(), ".runtime", "api-gateway-routing-state.json");
}
...
export class RoutingStateStore {
  ...
  constructor(config: AppConfig) {
    this.filePath = config.GATEWAY_ROUTING_STATE_FILE?.trim() || defaultStateFilePath();
  }
